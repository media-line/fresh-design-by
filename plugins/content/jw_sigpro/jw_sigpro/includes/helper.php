...
class SimpleImageGalleryProHelper {
    ...
    $siteUrl  = JURI::root(true).'/';
		...
		$srcFolder = array();
		$srcFoldersTmp = JFolder::folders($sitePath.$srcimgfolder);
		$srcFolders = array();
		$srcFolders[] = '';
		foreach ($srcFoldersTmp as $folder)
		 $srcFolders[] = $folder;
		foreach ($srcFolders as $folder){
			if ($folder != 'thumb'){
			$srcFolderTmp = JFolder::files($sitePath.$srcimgfolder.DS.$folder);
			foreach ($srcFolderTmp as $src)
			 $srcFolder[] = ($folder?$folder.DS:'').$src;
			}
		}
		...
		if ($menu->getActive() == $menu->getDefault())  
		 $total_count = 50;
		else $total_count = 1000;
				
		foreach($found as $key=>$filename){
			//echo $key.'<br/>';
			if ($key == $total_count) break;
			...
			$thumbimage = $cacheFolderPath.DS.$prefix.$gal_id.'_'.strtolower(SimpleImageGalleryProHelper::replaceWhiteSpace(str_replace(DS,'_',$thumbfilename)));
			//echo $thumbimage.'<br/>';
			...
			$gallery[$key]->sourceImageFilePath = $siteUrl.$srcimgfolder.'/'.SimpleImageGalleryProHelper::replaceWhiteSpace(str_replace(DS,'/',$filename));
			$gallery[$key]->thumbImageFilePath = $siteUrl.'cache/jw_sigpro/'.$prefix.$gal_id.'_'.strtolower(SimpleImageGalleryProHelper::replaceWhiteSpace(str_replace(DS,'_',$thumbfilename)));
