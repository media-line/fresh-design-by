...
$(window).addEvent( 'load', function(){
	...
	$$('.it-addrow-block .add').each( function( item, idx ){ 
		...
		var name2   = "jform[params]["+item.getProperty('id').replace('btna-','')+"][]"	
		var newrow = new Element('div', {'class':'row nerow'} );	
		var input  = new Element('input', {'name':name,'type':'text', 'class':'serty2'} );
    	var fut20  = new Element('textarea', {'name':name2, 'class':'serty3'} );
	
		var span   = new Element('span',{'class':'remove'});						
		var spantext   = new Element('span',{'class':'nummer'}); 
			  ...
			  //spantext.innerHTML= parent.getElements('input').length;	
		   	  spantext.innerHTML="<span class=\"nummer\"></span><div class=\"title01\">Введите координаты  и описания метки</div>";
		
			span.addEvent('click', function(){ 
			//Внимание, из БД удаляю выбранный элемент								
	...
	$$('.it-addrow-block .row').each(function( item ){
		item.getElement('.remove').addEvent('click', function() {
			if( item.getElement('input').value ) {
				if( confirm('Внимание, из БД удаляю выбранный элемент') ) {
					item.dispose();
				}
			}else {
				item.dispose();
